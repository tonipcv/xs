...
`DataType` define os tipos primários compatíveis com o Sidecar e a plataforma Xase:

```python
from xase.sidecar import SidecarDataset, DataType

ds = SidecarDataset(
    segment_ids=["seg_0001","seg_0002"],
    data_type=DataType.AUDIO,   # ou "AUDIO"
)
```

## Áudio além de WAV

Carregue bytes de áudio (WAV/FLAC/OGG/MP3) para tensor float32 mono e faça reamostragem para 16kHz (opcional):

```python
from xase.sidecar import audio_bytes_to_tensor

# data = open("sample.flac", "rb").read()
wav, sr = audio_bytes_to_tensor(data, target_sample_rate=16000)
```

Dependências opcionais:

- `soundfile` (libsndfile) – caminho preferencial
- `torchaudio` – fallback

Se nenhuma estiver instalada, uma exceção informativa será lançada.

## Pipeline config do Sidecar

Descubra dinamicamente as modalidades e transforms ativos no pipeline:

```python
from xase.sidecar import SidecarControl

ctl = SidecarControl(api_key="xase_live_...", base_url="https://xase.ai")
cfg = ctl.get_pipeline_config()
print(cfg["modalities"])     # ["AUDIO", "IMAGE", ...]
print(cfg.get("transforms")) # {"AUDIO": ["wm_detect", ...], ...}
```
