  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Fetch metrics (usando rota v1 supplier/client baseado no tipo de organização)
        const metricsRes = await fetch(`/api/v1/metrics/supplier?period=${period}`)
        if (!metricsRes.ok) throw new Error('Failed to fetch metrics')
        const metricsData = await metricsRes.json()
        setMetrics(metricsData)

        // Fetch alerts
        const alertsRes = await fetch('/api/v1/alerts?status=OPEN&limit=10')
