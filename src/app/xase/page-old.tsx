...
export default async function XaseDashboard({ searchParams }: { searchParams?: Promise<{ range?: string }> }) {
  ...
  if (tenantId) {
    [totalRecords, totalExports] = await Promise.all([
    ...
    // Simplified integrity status: if there are records, ledger is operational
    integrityStatus = totalRecords > 0 ? 'Verified' : 'Pending';
  }
  ...
  if (tenantId) {
    const [decisionTs] = await Promise.all([
  ...
  // HITL and Insurance metrics (last 24h)
  const periodMs = 24 * 60 * 60 * 1000;
  const start24h = new Date(now.getTime() - periodMs);
  ...
  if (tenantId) {
    const whereClause: any = {
      tenantId,
      timestamp: { gte: start24h, lte: now },
    };
    ...
    const [
      ...
      prisma.humanIntervention.groupBy({ by: ['action'], where: { tenantId, timestamp: { gte: start24h, lte: now } }, _count: true }),
      prisma.decisionRecord.aggregate({ where: whereClause, _avg: { confidence: true } }),
      prisma.insuranceDecision.aggregate({
        where: { record: { tenantId, timestamp: { gte: start24h, lte: now } } },
        _count: true,
        _avg: { claimAmount: true },
      }),
      prisma.insuranceDecision.count({
        where: { record: { tenantId, timestamp: { gte: start24h, lte: now } }, decisionOutcome: 'APPROVED' },
      }),
      prisma.insuranceDecision.count({
        where: { record: { tenantId, timestamp: { gte: start24h, lte: now } }, decisionImpactConsumerImpact: 'HIGH' },
      }),
      prisma.humanIntervention.findMany({
        where: { tenantId, action: 'OVERRIDE', timestamp: { gte: start24h, lte: now } },
        select: { reason: true },
    ...
    overrideCount = interventionsByAction.find((i: any) => i.action === 'OVERRIDE')?._count || 0;
    approvalCount = interventionsByAction.find((i: any) => i.action === 'APPROVED')?._count || 0;
    rejectionCount = interventionsByAction.find((i: any) => i.action === 'REJECTED')?._count || 0;
    ...
    avgClaimAmount = (insuranceAgg as any)._avg?.claimAmount ? Number((insuranceAgg as any)._avg.claimAmount) : null;
  ...
  // Checkpoints removed: single series only
    ...
    <AppLayout>
      <div className="min-h-screen bg-[#121316]">
        <div className="max-w-[1400px] mx-auto px-8 py-8 space-y-8">
            ...
            <div className="bg-white/[0.02] border border-white/[0.06] rounded-lg p-6">
              ...
              <div className="w-full overflow-visible">
                <svg width={sparkWidth} height={sparkHeight} viewBox={`0 0 ${sparkWidth} ${sparkHeight}`} className="w-full h-[360px] md:h-[420px]">
                  ...
                  />
                  {/* Records line */}
                  <polyline points={decisionPoints} fill="none" stroke="#94a3b8" strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" />
                  {/* Single series only */}
                  {/* Tick labels */}
                  {ticks.map((t, idx) => {
                    const isFirst = t.i === 0;
          ...
          {/* HITL Overview */}
          <div className="space-y-3">
            <h2 className="text-sm font-medium text-white/50">HITL overview (24h)</h2>
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              <div className="bg-white/[0.02] border border-white/[0.04] rounded-lg p-4">
                ...
                <p className="text-2xl font-semibold text-white mt-1">{totalDec}</p>
                <p className="text-xs text-white/50">{aiDecisions} AI · {humanInterv} Human</p>
              </div>

              <div className="bg-white/[0.02] border border-white/[0.04] rounded-lg p-4">
                <p className="text-xs font-medium text-white/80">Override Rate</p>
                <p className="text-2xl font-semibold text-white mt-1">{overrideRate.toFixed(1)}%</p>
                <p className="text-xs text-white/50">{overrideCount} overrides</p>
              </div>

              <div className="bg-white/[0.02] border border-white/[0.04] rounded-lg p-4">
                <p className="text-xs font-medium text-white/80">Avg Confidence</p>
              ...
              <div className="bg-white/[0.02] border border-white/[0.04] rounded-lg p-4 lg:col-span-1 md:col-span-2">
                <p className="text-xs font-medium text-white/80">Approval Rate (HITL)</p>
                <p className="text-2xl font-semibold text-white mt-1">{approvalRate.toFixed(1)}%</p>
                <p className="text-xs text-white/50">{approvalCount} approvals</p>
              </div>
            </div>
          </div>
          ...
          {/* Decisions by Source */}
          <div className="bg-white/[0.02] border border-white/[0.04] rounded-lg p-4">
            <p className="text-sm font-medium text-white/80 mb-2">Decisions: AI vs Human (24h)</p>
            <div className="space-y-2">
              ...
              <div className="h-2 w-full bg-white/10 rounded-full overflow-hidden">
                <div className="h-full bg-white/70" style={{ width: `${totalDec > 0 ? (aiDecisions / totalDec) * 100 : 0}%` }} />
              </div>
              ...
              <div className="h-2 w-full bg-white/10 rounded-full overflow-hidden">
                <div className="h-full bg-white/30" style={{ width: `${totalDec > 0 ? (humanInterv / totalDec) * 100 : 0}%` }} />
              </div>
            </div>
          </div>

          {/* Insurance + Top Reasons (compact) */}
          <div className="grid gap-3 md:grid-cols-2">
            {/* Insurance Metrics */}
            <div className="space-y-3">
              <h3 className="text-sm font-medium text-white/50">Insurance (24h)</h3>
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="bg-white/[0.02] border border-white/[0.04] rounded-lg p-4">
                  <p className="text-xs font-medium text-white/80">Total Claims</p>
                  <p className="text-xl font-semibold text-white mt-1">{totalClaims}</p>
                  <p className="text-[11px] text-white/50">Last 24 hours</p>
                </div>
                <div className="bg-white/[0.02] border border-white/[0.04] rounded-lg p-4">
                  <p className="text-xs font-medium text-white/80">Approval Rate</p>
                  <p className="text-xl font-semibold text-white mt-1">{insuranceApprovalRate.toFixed(1)}%</p>
                  <p className="text-[11px] text-white/60">↑ {insuranceApprovals} approved</p>
                </div>
                <div className="bg-white/[0.02] border border-white/[0.04] rounded-lg p-4">
                  <p className="text-xs font-medium text-white/80">Avg Claim Amount</p>
                  <p className="text-xl font-semibold text-white mt-1">
                    {avgClaimAmount ? `£${avgClaimAmount.toLocaleString('en-GB', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}` : 'N/A'}
                  </p>
                  <p className="text-[11px] text-white/50">Average payout</p>
                </div>
                <div className="bg-white/[0.02] border border-white/[0.04] rounded-lg p-4">
                  <p className="text-xs font-medium text-white/80">High Impact</p>
                  <p className="text-xl font-semibold text-white mt-1">{highImpactCount}</p>
            ...
            <div className="space-y-3">
              <h3 className="text-sm font-medium text-white/50">Top Override Reasons (24h)</h3>
              <div className="bg-white/[0.02] border border-white/[0.04] rounded-lg p-4">
                {topOverride.length > 0 ? (
                  ...
                  <p className="text-xs text-white/50">No overrides in the last 24 hours.</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              {/* Checkpoints quick action removed */}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              {/* Checkpoints status card removed */}
              <div className="flex flex-col gap-2 p-5 bg-white/[0.02] border border-white/[0.06] rounded-lg">
