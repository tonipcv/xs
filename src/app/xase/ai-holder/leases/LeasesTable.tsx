...
export function LeasesTable({ initialLeases, initialTotal, tenantId }: LeasesTableProps) {
  ...
  const getStatusBadge = (status: string) => {
    const colors = {
      ...
      EXPIRED: 'text-white/40 border-white/20 bg-white/5',
      REVOKED: 'text-red-400 border-red-400/30 bg-red-400/10',
    };
    return (
      <span className={`text-[10px] px-2 py-0.5 rounded border ${colors[status as keyof typeof colors] || 'text-gray-600 border-white/20'} uppercase font-medium tracking-wide`}>
        {status}
      </span>
    );
  };
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className={`text-xs font-medium ${
                        isActive && timeLeft !== 'Expired' ? 'text-gray-900' : 'text-white/40'
                      }`}>
                        {timeLeft}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      {isActive ? (
                        <button
                          ...
                          className="inline-flex items-center px-3 py-1.5 border border-red-400/30 bg-red-400/10 text-red-400 rounded text-xs font-medium hover:bg-red-400/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                        ...
                        <div className="text-xs text-gray-500">
                          <p>Revoked</p>
                          {lease.revokedReason && (
                            <p className="text-[10px] text-white/40 mt-0.5">{lease.revokedReason}</p>
                          )}
                        </div>
                      ) : (
