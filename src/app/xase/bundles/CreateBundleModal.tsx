  ...
  const handleDownloadBundle = async () => {
    ...
    try {
      const csrf = typeof document !== 'undefined'
        ? document.cookie.split('; ').find((c) => c.startsWith('x-csrf-token='))?.split('=')[1]
        : undefined;
      const dl = await fetch(`/api/v1/bundles/${currentBundleId}/download`, {
        method: 'POST',
        headers: {
          ...(csrf ? { 'x-csrf-token': csrf } : {}),
        },
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      const csrf = typeof document !== 'undefined'
        ? document.cookie.split('; ').find((c) => c.startsWith('x-csrf-token='))?.split('=')[1]
        : undefined;
      const res = await fetch(`/api/v1/bundles/${currentBundleId}/pdf`, {
        method: 'POST',
        headers: {
          ...(csrf ? { 'x-csrf-token': csrf } : {}),
        },
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {!created ? (
            ...
            <div className="space-y-3 pt-1">
              <p className="text-xs text-white/60">Bundle created successfully. View the compliance report to generate and download the PDF.</p>
              <button
                type="button"
                onClick={() => {
                  if (currentBundleId) {
                    window.open(`/xase/bundles/${currentBundleId}/pdf/preview`, '_blank');
                    onSuccess();
                    onClose();
                  }
                }}
                className="w-full px-3 py-2 bg-white text-black text-sm rounded-md transition-colors"
              >
                View Report
              </button>
            </div>
