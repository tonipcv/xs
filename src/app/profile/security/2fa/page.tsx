      ...
      <div className="min-h-screen bg-[#0e0f12]">
        <div className="max-w-[1100px] mx-auto px-8 py-8 space-y-8">
          <div className="space-y-1.5">
            <h1 className={`${heading.className} text-2xl font-semibold text-white tracking-tight`}>Two-Factor Authentication</h1>
            <p className="text-sm text-white/60">Use an authenticator app to add an extra layer of security to your account.</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white/[0.03] border border-white/[0.08] rounded-xl p-6">
                <div className="space-y-1 mb-4">
                  <h2 className={`${heading.className} text-sm font-semibold text-white/90`}>Setup</h2>
                  <p className="text-xs text-white/50">Use Google Authenticator, 1Password, Authy or similar.</p>
                </div>

                {!otpauthUrl ? (
                  <Button onClick={startSetup} disabled={loading} className="bg-white/[0.06] hover:bg-white/[0.12] text-white">
                    {loading ? 'Generating…' : 'Enable 2FA'}
                  </Button>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-white/[0.08] bg-white/[0.02] p-4">
                      {qrDataUrl && (
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2">
                        <Button onClick={verify} disabled={loading || token.length === 0} className="bg-white/[0.06] hover:bg-white/[0.12] text-white">Verify & enable</Button>
            ...
            <div className="space-y-6">
              <div className="bg-white/[0.03] border border-white/[0.08] rounded-xl p-6">
                <h3 className={`${heading.className} text-sm font-semibold text-white/90 mb-2`}>Tips</h3>
              ...
              <div className="bg-white/[0.03] border border-white/[0.08] rounded-xl p-6">
                <h3 className={`${heading.className} text-sm font-semibold text-white/90 mb-2`}>Back</h3>
