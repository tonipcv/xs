...
export async function GET(req: NextRequest, { params }: { params: Promise<{ datasetId: string }> }) {
  try {
    ...
    if (estimatedHours > 0) {
      await prisma.$transaction(async (tx) => {
        ...
        if (execution) {
          ...
          const newTotalCost = execution.totalCost + (estimatedHours * (policy.pricePerHour || 0))
    ...
    console.error('[API] datasets/:datasetId/stream error:', msg)
