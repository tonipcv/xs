  ...
  const handleRequestAccess = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => {
          router.push(`/app/executions/${data.execution.executionId}`)
        }, 1500)
      } else {
      ...
      <AppLayout>
        <div className="min-h-screen bg-gray-50 px-8 py-8">
          <div className="max-w-[1200px] mx-auto text-center py-12">
            ...
            <h2 className="text-2xl font-semibold mb-2 text-gray-900">Offer Not Found</h2>
            <p className="text-sm text-gray-600">The access offer you're looking for doesn't exist.</p>
          </div>
        </div>
      </AppLayout>
    )
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-8 py-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className={`${hasSidebar ? 'lg:col-span-2' : 'lg:col-span-3'} space-y-6`}>
              ...
              {/* Manage Offer - only for suppliers */}
              {tenantId && isSupplier && (
                <Card className="bg-white border border-gray-200 rounded-xl">
                  <CardHeader>
                  ...
                  <CardContent className="space-y-2 text-sm">
                    <Button variant="outline" className="w-full border-gray-300 text-gray-900 hover:bg-gray-50" onClick={() => router.push(`/app/marketplace/publish?datasetId=${offer?.dataset?.datasetId || ''}`)}>Republish / New Version</Button>
                    <Button variant="outline" className="w-full border-gray-300 text-gray-900 hover:bg-gray-50" onClick={() => router.push(`/app/policies`)}>View Policies</Button>
                    <Button variant="outline" className="w-full border-gray-300 text-gray-900 hover:bg-gray-50" onClick={() => router.push(`/app/leases`)}>View Leases</Button>
                  </CardContent>
                </Card>
              )}
            </div>
            ...
            {hasSidebar && (
              <div className="space-y-6">
                {/* Request Access - only for buyers (not supplier). Wait for tenantId to avoid flicker. */}
                {tenantId && !isSupplier && (
                  <Card className="bg-white border border-gray-200 rounded-xl">
                    <CardHeader>
                    ...
                    <CardContent className="space-y-4">
                      {!showRequestModal ? (
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            <label className="text-sm font-medium mb-2 block text_gray-900">Requested Hours (optional)</label>
                            <Input
                              type="number"
