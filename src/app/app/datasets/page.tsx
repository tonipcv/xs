      ...
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-[1400px] mx-auto px-8 py-8 space-y-8">
          ...
          <div className="flex items-center gap-3 text-sm">
            <Link href="/app/datasets/new" className="text-gray-600 hover:text-gray-900 underline-offset-2 hover:underline">Create dataset</Link>
          </div>
          ...
          {integrations.filter((i) => i.status === 'ACTIVE').length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Link href="/app/settings" className="text-xs text-gray-500 hover:text-gray-800">Manage</Link>
              </div>
                ...
                {integrations.filter((i) => i.status === 'ACTIVE').map((intg) => (
                  <div key={intg.id} className="bg-white border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="mt-3 flex items-center justify-end">
                      <Link href={`/app/datasets`} className="text-xs text-gray-700 hover:text-gray-900 underline">
                        Browse
                      </Link>
                    </div>
                  </div>
