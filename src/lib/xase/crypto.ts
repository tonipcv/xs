...
export function isValidApiKey(key: string): boolean {
  // Accept legacy hex keys and new URL-safe keys (base64url-like) after the prefix
  // Examples:
  // - xase_pk_6f1a... (48 hex chars)
  // - xase_pk_QpB-b7kP8Yit_Y5GLR0lzlNS3MDrv7fM (URL-safe chars)
  return (
    /^xase_pk_[A-Za-z0-9_-]{24,128}$/.test(key) ||
    /^xase_pk_[a-f0-9]{48}$/.test(key) ||
    // Also accept keys without the "pk" segment (observed in some envs)
    /^xase_[A-Za-z0-9_-]{24,128}$/.test(key)
  );
}
