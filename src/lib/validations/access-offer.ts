...
export const AccessConstraintsSchema = z.object({
  ...
  maxConcurrentStreams: z.coerce.number().positive().optional(),
  rateLimit: z.string().trim().optional(),
  retentionPolicy: z.string().trim(),
...
export const CreateAccessOfferSchema = z.object({
  ...
  scopeHours: z.coerce.number().positive(),
  scopeRecordings: z.coerce.number().int().positive().optional(),
