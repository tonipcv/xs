...
export async function middleware(request: NextRequest) {
  ...
  // Redirect legacy /xase/voice routes to /xase/ai-holder
  if (normalizedPath.startsWith('/xase/voice') && !normalizedPath.startsWith('/xase/voice/setup')) {
    const newPath = normalizedPath.replace('/xase/voice', '/xase/ai-holder');
    const url = new URL(newPath, request.url);
    const res = NextResponse.redirect(url);
    res.headers.set('X-Redirect-Reason', 'legacy_voice_redirect');
    res.headers.set('X-Req-Id', reqId);
    console.log(JSON.stringify({ tag: 'mw_redirect', reqId, reason: 'legacy_voice_redirect', from: pathname, to: newPath }));
    return applySecurityHeaders(res);
  }
